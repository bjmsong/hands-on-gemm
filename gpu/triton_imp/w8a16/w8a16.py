...
def int8_weight_only_linear_kernel(
    ...
    s = tl.load(s_ptr)
    y = (accumulator * s / 127.0).to(tl.bfloat16)
...
w = torch.randn(N, D, dtype=torch.bfloat16).cuda()
...
result = triton.testing.do_bench(lambda: int8_weight_only_linear(x, w_int8, scale), quantiles=[0.5, 0.2, 0.8])[0]
print(result)  